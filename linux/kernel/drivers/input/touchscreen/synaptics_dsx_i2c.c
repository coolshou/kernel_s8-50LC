		...
		struct device_attribute *attr, char *buf)
{
	...
	unsigned char firmid[4];
	unsigned char moduleid[2];
	int retval;
	/* read Customer Defined Config ID  */
	retval = synaptics_rmi4_i2c_read(rmi4_data,
		0x0050,
		firmid,
		sizeof(firmid));
	/* read Customer family module ID  */
	retval = synaptics_rmi4_i2c_read(rmi4_data,
		0x00A4,
		&moduleid,
		sizeof(moduleid));
	return sprintf(buf, "ID0=%02x,ID1=%02x,ID2=%02x,ID3=%02x\nModule ID=%02x\npanel_type=%02x\n",
		   firmid[0],firmid[1],firmid[2],firmid[3],moduleid[0],moduleid[1]);
}
 ...
 static void  synaptics_rmi4_late_init_work(struct work_struct *work)
{
	...
	if (platform_data->gpio_config) {		//synaptics_gpio_setup
		...
		if(!borad_id0_value)
		{
			...
			lnw_gpio_set_pininfo(88,TYPE_PULLMODE,"pulldown");
			lnw_gpio_set_pininfo(89,TYPE_PULLMODE,"pulldown");		
			gpio_set_value(platform_data->reset_gpio, 0);
			msleep(20);
			if (platform_data->power_gpio >= 0) {
				gpio_set_value_cansleep(platform_data->power_gpio, 0);
				msleep(160);
			}
			msleep(400);
			//power up
...
err_set_input_dev:
	if (platform_data->gpio_config && (platform_data->reset_gpio >= 0))
		platform_data->gpio_config(platform_data->reset_gpio, false,0 ,0);
err_gpio_reset:
	if (platform_data->gpio_config && (platform_data->power_gpio >= 0))
		platform_data->gpio_config(platform_data->power_gpio, false, 0, 0);
err_gpio_power:
	if (platform_data->gpio_config && (platform_data->power_gpio >= 0))
		platform_data->gpio_config(platform_data->irq_gpio, false, 0, 0);		
err_gpio_irq:
