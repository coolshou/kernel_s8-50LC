	...
	struct net_device *dev, char *country_code, bool notify, bool user_enforced)
{
	...
	if ((error < 0) ||
		...
		cspec.rev = -1;
              if (!strncmp(country_code, "MX", strlen("MX"))){
                    printk("country_code is %s !\n",country_code);
                    country_code = "US";
                    printk("country code is Mexico is %s !\n", country_code);
              }
              if ((!strncmp(country_code, "RU", strlen("RU")))
                || (!strncmp(country_code, "UA", strlen("UA")))
                || (!strncmp(country_code, "TZ", strlen("TZ")))
                || (!strncmp(country_code, "KR", strlen("KR")))
                || (!strncmp(country_code, "UG", strlen("UG")))
                || (!strncmp(country_code, "NG", strlen("NG")))){
                    printk("country_code is %s !\n",country_code);
                    country_code = "GB";
                    printk("country code is changed to %s !\n", country_code);
              }
              if (!strncmp(country_code, "IR", strlen("IR")))	return 0;
